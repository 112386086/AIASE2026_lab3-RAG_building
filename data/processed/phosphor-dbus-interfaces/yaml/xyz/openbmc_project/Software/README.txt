...
`/xyz/openbmc_project/software/ ` object. In addition, the
...
`Software.Activation` under `/xyz/openbmc_project/software/ `. Application of
...
`Software.Version` instance (ie. `/xyz/openbmc_project/software/ `). This is
...
the corresponding image, such as `/xyz/openbmc_project/software/ / `.
...
implementation specific, algorithm for the ` ` portion of a D-Bus path for
...
- ` ` corresponds to the organization owning the ` `, such as
  `xyz.openbmc_project` or `com.foo_corp`.
- ` ` is a unique name for the element, such as `SystemFoo` or
  `BoardBar`. Typically these would be code names for the hardware element such
  as `Witherspoon`.
- ` ` is an identifier for sub-element the image corresponds to and is
  documented in the ` /Software/Element/ .interface` file under
  the `Type` enumeration.

The following ` ` are reserved for a particular meaning:
...
collect an image once all `Software.Activation` under the `.../software/ `
...
List `/xyz/openbmc_project/inventory/.../ /activation` association.
