...
E.g. `/xyz/openbmc_project/network/ `
...
`/xyz/openbmc_project/network/ /ipv4/ `

IPv6 objects will have the following D-Bus object path:

`/xyz/openbmc_project/network/ /ipv6/ `
...
curl -c cjar -b cjar -k -H "Content-Type: application/json" -X  POST \
    ...
    https://${bmc}/xyz/openbmc_project/network/ /action/IP
```
...
curl -c cjar -b cjar -k -H "Content-Type: application/json" -X DELETE \
    https://${bmc}/xyz/openbmc_project/network/ /ipv4/ 
```
...
```sh
curl -c cjar -b cjar -k -H "Content-Type: application/json" \
    https://${bmc}/xyz/openbmc_project/network/ /attr/MACAddress
```
...
```sh
busctl call xyz.openbmc_project.Network /xyz/openbmc_project/network \
    xyz.openbmc_project.Network.VLAN.Create VLAN su " "  
```
...
```sh
curl -c cjar -b cjar -k -H "Content-Type: application/json" -X DELETE \
    https://${bmc}/xyz/openbmc_project/network/ 
```
...
ipmitool -I dbus lan set 1 ipaddr  

ipmitool -I dbus lan set 1 netmask  

ipmitool -I dbus lan set 1 defgw ipaddr  

ipmitool -I dbus lan set 1 vlan id  
